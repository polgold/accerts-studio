...
export async function GET(request: NextRequest) {
  ...
  if (!url || !key) {
    return NextResponse.json(
      { error: 'Supabase env not configured', hasUser: false, userId: null, email: null },
      { status: 503 }
    );
  }
  const response = NextResponse.json({ hasUser: false, userId: null, email: null });
  const supabase = createServerClientForRouteHandler(request, response);
  const { data: { user } } = await supabase.auth.getUser();
  const body = { hasUser: !!user, userId: user?.id ?? null, email: user?.email ?? null };
